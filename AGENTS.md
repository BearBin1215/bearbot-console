...
- 应当审查有无反复补丁迹象，模块复杂度提高到一定程度、或补丁痕迹明显时应考虑是否重构

## 萌娘百科 API 限制

萌百 API 有一定反爬限制，通常根据内容敏感度、编辑者是否有用等多方面综合裁定，包括：

- 限制未登录用户访问，如获取页面源代码（本工程的`getPageSource`方法）
- 限制非机器人、非维护组成员（管理员、维护姬）访问，如请求分类成员（本工程的`fetchCategoryMembers`方法）
- 限制非管理员访问，如获取所有版本记录（`list=allrevisions`）

在制定任务方案时，如果需要通过 curl 发出请求验证可行性，通常应该在维基百科等其他站点上测试；涉及爬虫的请求应当让用户确认其账号是否可用。
